#!/usr/bin/env python3
"""
Daily 15-Minute Settlement Point Prices Collector

Automatically collects 15-minute Settlement Point Prices for the previous day.
...
Example cron entry (runs daily at 1 AM):
    0 1 * * * cd /path/to/ercot-api-query && python3 scripts/daily_spp_15min.py
"""
...
def collect_spp_15min(debug=False):
    """
    Collect 15-minute Settlement Point Prices for yesterday.
    ...
    print("=" * 60)
    print("Daily 15-Minute Settlement Point Prices Collection")
    print("=" * 60)
    ...
    # Create output directory structure: output/daily/spp/YYYY/MM/
    output_dir = Path("output/daily/spp") / date_str[:4] / date_str[5:7]
    output_dir.mkdir(parents=True, exist_ok=True)

    # Generate output filename: spp_15min_2025-01-27.json
    output_file = output_dir / f"spp_15min_{date_str}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Collect 15-minute Settlement Point Prices for yesterday',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Collect 15-minute SPP data for yesterday
...
Note:
  This script collects Settlement Point Prices at 15-minute intervals
  for all nodes, zones, and hubs for the entire previous day.
  This can be a large dataset.
    ...
    # Run collection
    success = collect_spp_15min(debug=args.debug)
